...
# (Ported from the gold 0.25.1 lineage, TKhO7 mtk_grid_func.jl:3-16.) Rewrites the
# generated function's AST *after* codegen: (1) appends the coordinate argument
# symbols to the signature; (2) replaces every literal `_CoordTmpF` call site with
# the matching coordinate argument. Operating post-codegen is load-bearing for
# large mechanisms: rewriting the symbolic expressions *before* codegen
# (`Symbolics.substitute` on every sparse-Jacobian entry + `fixpoint_sub`
# pre-inlining of all observed equations — the previous scheme here) rebuilds
# every expression tree and destroys the `===` node sharing that the
# MTKBase-1.42/SymbolicUtils fast_toexpr backend relies on for deduplication.
# For the 274-species GEOS-Chem mechanism that produced a generated sparse-jac
# body of ~10^5-10^6 statements whose first-call compilation overflowed the task
# stack (HPC StackOverflowError in jac_stiff); the 19-species SuperFast stayed
# under the threshold, which is why SF worked while GC crashed.
function rewrite_coord_func(x, coord_args, idv::Symbol)
    ...
    elseif @capture(x, (a_)(b_))
        # PORT NOTE (MTKBase 1.42 fast_toexpr): the independent variable may be
        # renamed to a hashed argument symbol (`__argₛᵧₘ<hash>`), so unlike the
        # gold original we do NOT also require `b == idv` — a literal `_CoordTmpF`
        # instance in call position is unambiguous by itself.
        if a isa _CoordTmpF
